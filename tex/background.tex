...
In their future-work discussion, Krokos et al. also called for studies in which users build, manipulate, and organise their own palaces, and for a comparison between highly personal and more widely usable palaces~\cite{krokos2019}. Their experiment did not test those proposals. The distinction matters here: the present generic-versus-home comparison addresses the personal-space question, while active authoring and familiarity remain combined in its personalised condition.
...
Moll and Sykes let participants choose among images for each word and suggest that allowing an unrestricted personal image choice could be investigated further~\cite{moll2023}. That is personalisation of the mnemonic cue, whereas this thesis varies the spatial setting. Separating those choices helps identify what a later experiment would need to control.
...
Perera et al.~\cite{perera2019} described a customisable mobile VR memory-palace prototype and proposed offering different palace templates according to user preference. This supports personalisation as a system-design direction, but their short conference abstract does not establish a recall benefit from preference-based templates. A home reconstruction is also more specific than choosing a preferred generic template: it brings prior knowledge of the route into the virtual environment.
...
Perera et al.~\cite{perera2019} & Customisable mobile VR palace & Proposes preference-based templates; no tested home-layout effect. \\
